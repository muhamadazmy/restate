...
impl ConnectionInfo {
    ...
    pub async fn get_logs(&self) -> Result<Logs, ConnectionInfoError> {
        ...
        let cluster_size = nodes_config.iter().count();
        // try 50% + 1 of cluster size to find the latest logs version
        let try_nodes = nodes_config
            .iter()
            .map(|(_, node)| &node.address)
            .choose_multiple(&mut rand::thread_rng(), (cluster_size / 2) + 1);
