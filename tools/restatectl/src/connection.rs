...
impl ConnectionInfo {
    ...
    /// Gets NodesConfiguration object. This function will try `try_best_of` nodes from the provided
    /// addresses. Worst case all addresses will be tried if some or all nodes did not satisfy the query
    async fn get_latest<T, M>(
        &self,
