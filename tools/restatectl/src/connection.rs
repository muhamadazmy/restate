...
impl ConnectionInfo {
    ...
    /// Gets Logs object. This function tries all provided addresses and makes sure
    /// nodes logs is cached.
    pub async fn get_logs(&self) -> Result<Logs, ConnectionInfoError> {
        ...
        let nodes_addresses = nodes_config
            .iter()
        ...
        T: StorageDecode + Versioned + Clone,
    {
        ...
        for address in addresses {
            ...
            // todo(azmy): can we this get_indent call and directly
            // go for the get_metadata? it will return a version INVALID
            // and we will know it haven't been initialized!
            answer = true;
            ...
            if Version::from(response.nodes_config_version)
                <= latest_meta
                    .as_ref()
                    .map(|c| c.version())
                    .unwrap_or(Version::INVALID)
            {
                // has older version that we have.
                continue;
            }
            ...
            let meta = StorageCodec::decode::<T, _>(&mut response.encoded).map_err(|err| {
                ConnectionInfoError::InvalidNodesConfiguration(address.clone(), err)
            })?;
