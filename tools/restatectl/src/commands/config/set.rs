...
async fn config_set(connection: &ConnectionInfo, set_opts: &ConfigSetOpts) -> anyhow::Result<()> {
    ...
    if let Some(partition_replication) = &set_opts.partition_replication {
        let (partition_replication_kind, partition_replication) =
            partition_replication.clone().into_proto_parts();

        current.partition_replication = partition_replication;
        current.partition_replication_kind = partition_replication_kind.into();
    }
