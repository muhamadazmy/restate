...
async fn aggregate_status(row: &mut NodeRow, node: &AliveNode, logs: &Logs) -> anyhow::Result<()> {
    ...
    row.with_logs(sequencers, optimal);
...
impl NodeRow {
    ...
    fn with_logs(&mut self, count: usize, optimal: bool) -> &mut Self {
        let mut cell = Cell::new(count).fg(Color::Yellow);
        if optimal {
