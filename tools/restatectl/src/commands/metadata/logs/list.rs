...
use crate::commands::metadata::{get::get_value, MetadataCommonOpts};
...
async fn get_logs(opts: &ListLogsOpts) -> anyhow::Result<()> {
    let logs: Logs = get_value(&opts.metadata, "bifrost_config")
        .await?
    ...
    for (log_id, chain) in logs {
        logs_table.add_row(vec![
            ...
            Cell::new(format!("{}", *chain.tail().config.params)),
        ]);
    }
