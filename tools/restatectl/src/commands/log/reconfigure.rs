    ...
    let params = if tail_segment.config.kind == ProviderKind::Replicated {
        ...
        ReplicatedLogletParams {
            ...
            replication: ReplicationProperty::new(
                opts.replication.context("Missing replication-factor. Replication factor is required if last segment is not of replicated type")?,
            ),
