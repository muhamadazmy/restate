...
use crate::metric_definitions::{
    METADATA_SERVER_EMBEDDED_APPLIED_LSN, METADATA_SERVER_EMBEDDED_COMMITTED_LSN,
    METADATA_SERVER_EMBEDDED_FIRST_INDEX, METADATA_SERVER_EMBEDDED_LAST_INDEX,
    METADATA_SERVER_EMBEDDED_LEADER_ID, METADATA_SERVER_EMBEDDED_SNAPSHOT_SIZE_BYTES,
...
impl Member {
    ...
    fn record_summary_metrics(&self, summary: &MetadataStoreSummary) {
        ...
        if let Some(id) = leader {
            gauge!(METADATA_SERVER_EMBEDDED_LEADER_ID).set(u32::from(*id) as f64);
        } else {
            gauge!(METADATA_SERVER_EMBEDDED_LEADER_ID).set(INVALID_ID as f64);
        }

        if let Some(snapshot) = snapshot {
            gauge!(METADATA_SERVER_EMBEDDED_SNAPSHOT_SIZE_BYTES).set(snapshot.size as f64);
        } else {
            gauge!(METADATA_SERVER_EMBEDDED_SNAPSHOT_SIZE_BYTES).set(0);
        }

        gauge!(METADATA_SERVER_EMBEDDED_TERM).set(raft.term as f64);
        gauge!(METADATA_SERVER_EMBEDDED_APPLIED_LSN).set(raft.applied as f64);
        gauge!(METADATA_SERVER_EMBEDDED_COMMITTED_LSN).set(raft.committed as f64);
        gauge!(METADATA_SERVER_EMBEDDED_FIRST_INDEX).set(raft.first_index as f64);
        gauge!(METADATA_SERVER_EMBEDDED_LAST_INDEX).set(raft.last_index as f64);
    }
