...
impl<M: Send + Sync + 'static> StorageEncode for Envelope<M> {
    ...
    fn encode(&self, buf: &mut BytesMut) -> Result<(), StorageEncodeError> {
        // since we using Custom codec it's important to be able to
        // update the encoding structure internally without breaking old
        // code. Hence we should track encoding version.
        ...
        Self: Sized,
    {
        match kind {
            ...
            StorageCodecKind::Custom => {
                ...
                let inner = Header::decode_length_delimited(&mut *buf)
...
mod test {
    ...
    use super::{Dedup, Destination, Envelope, Source, records};
    use crate::{
