...
impl<R> HasRecordKeys for Envelope<R>
where
    R: Record,
...
impl<R: Record + 'static> StorageEncode for Envelope<R> {
...
pub trait Record: sealed::Sealed + Sized {
    ...
    fn new(source: Source, dedup: Dedup, payload: Self::Payload) -> Envelope<Self>
    where
        Self::Payload: StorageEncode,
    {
        Envelope::new(source, dedup, payload)
    }
}
