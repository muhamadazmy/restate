        ...
        fn try_from(value: IncomingEnvelope<Raw>) -> Result<Self, Self::Error> {
            ...
            let (header, _command) = match value.kind() {
                ...
                RecordKind::PurgeInvocation => {
                    value.into_typed::<records::PurgeInvocation>().split().map(
                        |(header, payload)| (header, v1::Command::PurgeInvocation(payload.into())),
                    )
                }
                ...
                RecordKind::AttachInvocation => {
                    value.into_typed::<records::AttachInvocation>().split().map(
                        |(header, payload)| (header, v1::Command::AttachInvocation(payload.into())),
                    )
                }
                RecordKind::ResumeInvocation => {
                    value.into_typed::<records::ResumeInvocation>().split().map(
                        |(header, payload)| (header, v1::Command::ResumeInvocation(payload.into())),
                    )
                }
                ...
                RecordKind::InvocationResponse => value
                    ...
                    .split()
                    .map(|(header, payload)| {
                        (header, v1::Command::InvocationResponse(payload.into()))
                    }),
