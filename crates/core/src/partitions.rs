...
use crate::{
    cancellation_watcher, Metadata, ShutdownError, TargetVersion, TaskCenter, TaskId, TaskKind,
};
                ...
                Some(cmd) = self.receiver.recv() => {
                    match cmd {
                        Command::SyncRoutingInformation => {
                            _ = TaskCenter::spawn_child(TaskKind::Disposable, "routing-partition-table-sync", async {
                                Metadata::current()
