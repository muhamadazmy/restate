        ...
        match body {
                ...
                match body {
                    stream_outbound::Body::Payload(payload) => {
                        ...
                        if let Err(err) = self
                            ...
                            .handle_message(&stream.stream_id, StreamEnvelope::Payload(payload))
                        {
                    ...
                    stream_outbound::Body::Status(status) => {
                        ...
                        if let Err(err) = self
                            .shared
                            .local_initiated_streams
                            .handle_message(&stream.stream_id, StreamEnvelope::Status(status))
                            && !peer_dropped
                        {
                            // only send back the error if peer is not
                            // already dropped
                            send_inbound_stream_error(tx, err, stream.stream_id);
                        }
            ...
            Body::Datagram(Datagram {
                ...
                spawn_stream_responder(
                    tx.clone(),
                    Arc::clone(&self.shared.remote_initiated_streams),
                    stream.stream_id,
                    outbound_rx,
                    target_service,
                );
...
fn spawn_stream_responder(
    ...
    id: u64,
    mut outbound_rx: mpsc::Receiver<StreamEnvelope>,
    _target_service: ServiceTag,
) {
    // this is rpc-call, spawning a responder task
    tokio::spawn(async move {
