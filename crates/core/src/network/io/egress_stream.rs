...
use crate::network::protobuf::network::{self, Datagram, StreamInbound, stream_inbound};
...
impl EgressMessage {
    ...
    pub fn make_open_stream_message<M: StreamRequest>(
        ...
        let (message_tx, message_rx) = mpsc::channel::<StreamEnvelope>(128);
    ...
    // makes a new inbound stream message for
    // the remote peer to handle
...
impl EgressStream {
    ...
    fn new(capacity: usize) -> (EgressSender, Self, super::Shared) {
        ...
        let remote_initiated_streams: Arc<StreamTracker> = Default::default();
    ...
    fn handle_message(
        ...
        match entry {
            ...
            Poll::Ready(Some(EgressMessage::OpenStreamCall {
                ...
                // let know the stream opener the stream id
