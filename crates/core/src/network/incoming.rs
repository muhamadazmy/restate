...
pub struct Sink<O: StreamResponse> {
    inner: mpsc::Sender<StreamEnvelope>,
    _phantom: PhantomData<O>,
}
...
pub(crate) struct StreamPort {
    ...
    outbound_tx: mpsc::Sender<StreamEnvelope>,
}

impl StreamPort {
    pub fn new(buffer: usize) -> (Self, StreamPeerPort) {
...
pub(crate) struct StreamPeerPort {
    outbound_rx: mpsc::Receiver<StreamEnvelope>,
    inbound_tx: mpsc::Sender<StreamEnvelope>,
}

impl StreamPeerPort {
    pub fn split(self) -> (mpsc::Sender<StreamEnvelope>, mpsc::Receiver<StreamEnvelope>) {
...
impl<M: StreamResponse> Reciprocal<Sink<M>> {
    pub async fn send(&self, message: M) -> Result<(), StreamClosed> {
        let bytes = message
    ...
    pub async fn fail(self, verdict: Verdict) -> Result<(), StreamClosed> {
        self.reply_port
            .inner
