...
use crate::metric_definitions::{INVOKER_TASK_DURATION, PARTITION_ID_LOOKUP};
    ...
    Schemas: DeploymentResolver + InvocationTargetResolver,
{
    ...
    pub async fn run(mut self, input_journal: InvokeInputJournal) {
        ...
        histogram!(INVOKER_TASK_DURATION, "partition_id" => PARTITION_ID_LOOKUP.get(self.partition.0)).record(start.elapsed());
    }
