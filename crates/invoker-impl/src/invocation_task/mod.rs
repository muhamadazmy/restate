...
pin_project_lite::pin_project! {
    ...
    enum ResponseStream {
        WaitingHeaders {
            join_handle: AbortOnDrop<Result<Response<ResponseBody>, ServiceClientError>>,
        },
        ReadingBody {
            #[pin]
            body: ResponseBody,
...
impl ResponseStream {
    fn initialize(client: &ServiceClient, req: Request<InvokerBodyStream>) -> Self {
        ...
        Self::WaitingHeaders {
            join_handle: AbortOnDrop(tokio::task::spawn(client.call(req))),
        }
    }
}
        ...
        match this {
            ...
            ResponseStreamProj::ReadingBody { body } => {
                ...
                match next_element.transpose() {
                    ...
                    Err(err) => Poll::Ready(Some(Err(InvokerError::ClientBody(err)))),
                }
            }
        }
    }
