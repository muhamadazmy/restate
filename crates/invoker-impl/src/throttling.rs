    ...
    C: gardal::Clock,
{
    ...
    pub fn new(bucket: super::TokenBucket<C>) -> Self {
        ...
        Self {
            bucket,
            state,
            sleep: sleep.map(|wait_time| tokio::time::sleep(wait_time.into())),
        }
    }
            ...
            .expect("1 is less than burst capacity")
        {
            ...
            Some(wait_time) => {
                *this.state = TokenStateInner::Waiting;
                this.sleep.set(Some(tokio::time::sleep(wait_time.into())));
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_token_state_immediate_ready() {
        let (bucket, _) = create_token_bucket_with_tokens(10);
        bucket.add_tokens(10);
    ...
    #[test]
    fn test_token_state_empty_to_waiting() {
        let (bucket, _) = create_token_bucket_with_tokens(10);
        let token_state = TokenState::new(bucket);
        tokio::pin!(token_state);
    ...
    fn test_token_state_consume_not_ready_panics() {
        let (bucket, _) = create_token_bucket_with_tokens(10);
        let token_state = TokenState::new(bucket);
        tokio::pin!(token_state);
    ...
    #[test]
    fn test_token_state_multiple_consumes() {
