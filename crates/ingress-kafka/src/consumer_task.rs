    ...
    C: ConsumerContext,
{
            ...
            tokio::select! {
                ...
                Some(committed) = Self::head_committed(&mut inflight) => {
                    ...
                    trace!(
                        ...
                        offset=%head.offset, consumer_group=%self.consumer_group_id,
                        "store kafka offset",
                    );
                ...
                received = self.topic_partition_consumer.recv() => {
                    ...
                    // not block
                    let mut ingest = std::pin::pin!(self.ingestion.ingest(envelope.partition_key(), envelope));
                    ...
                    inflight.push_back(InflightMessage{offset, commit_token});
                }
            }
        }
    }
    ...
    pub fn head_committed(
        ...
        OptionFuture::from(
            inflight
                .front_mut()
                .map(|i: &mut InflightMessage| &mut i.commit_token),
        )
    }
}
