        ...
        loop {
            tokio::select! {
                ...
                received = self.topic_partition_consumer.recv() => {
                    ...
                    // while trying to reserve capacity to send the message,
                    // we also make sure to process committed messages to release permits as soon as
                    // possible, which gives better chances for sending to
                    // not block

                    let mut reserve = std::pin::pin!(self.ingestion.reserve(msg.estimate_size() as u32));
                    ...
                    let token = permit.ingest(pk, msg)?;
