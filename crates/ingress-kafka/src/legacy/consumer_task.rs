...
async fn query_dedup_offset<T: TransportConnect>(
    ...
    while let Some((partition_id, result)) = fut.next().await {
        let retry_iter = backoff
            .entry(partition_id)
            .or_insert_with(|| retry_policy.clone().into_iter());
    ...
    let result = async {
        ...
        let result = match result {
            ...
            Err(RpcError::Receive(RpcReplyError::MessageUnrecognized)) => {
                ...
                // Once the cluster finishes rolling to v1.6, the endpoint should exist and this branch should not trigger.
                return Ok(None);
            }
