...
use crate::SERVICES_TRACING_TARGET;
...
#[derive(Debug)]
pub struct ForkExporter<A, B> {
    runtime: Arc<Mutex<A>>,
    services: Option<Arc<Mutex<B>>>,
}

impl<A, B> ForkExporter<A, B> {
...
impl<A, B> SpanExporter for ForkExporter<A, B>
where
        ...
        if self.services.is_some() {
            for span in batch.iter() {
                if span
                    .attributes
                    .iter()
                    .find(|kv| kv.key == USER_SERVICE_KEY)
                    .is_some()
                {
