...
pub struct LogsController {
    ...
    in_flight_refresh_tail: Option<TaskHandle<()>>,
}
...
impl LogsController {
    ...
    pub fn schedule_refresh_tails(&mut self) -> Result<(), ShutdownError> {
        if let Some(ref task) = self.in_flight_refresh_tail {
            if !task.is_finished() {
        ...
        self.in_flight_refresh_tail = Some(handle);
        Ok(())
    }
