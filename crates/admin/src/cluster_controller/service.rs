...
impl<T: TransportConnect> Service<T> {
    ...
    fn cluster_controller_status(&self) -> ClusterControllerStatus {
        ...
        let maybe_active = !maybe_active
            .is_some_and(|active_node_id| active_node_id != self.metadata.my_node_id().as_plain());

        if let Err(err) = nodes_config.find_node_by_id(self.metadata.my_node_id()) {
            warn!("Shuting down cluster controller because of a node generation error {err}");
            return ClusterControllerStatus::Shutdown;
        }

        match maybe_active {
            true => ClusterControllerStatus::Active,
            false => ClusterControllerStatus::Passive,
        }
    }
    ...
    pub async fn run(mut self) -> anyhow::Result<()> {
        ...
        let mut status = self.cluster_controller_status();
