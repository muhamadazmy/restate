...
pub struct ChainExtension {
    ...
    pub last_segment_index: Option<SegmentIndex>,
    /// Wait for logs min_version before sealing
...
impl SealAndExtendTask {
    async fn run(mut self) -> anyhow::Result<SealedSegment> {
        let last_segment_index = self
            .extension
            .as_ref()
            .and_then(|ext| ext.last_segment_index);
    ...
    async fn next_params(&self) -> anyhow::Result<(ProviderKind, LogletParams)> {
        ...
        let (provider, params) = match &logs.configuration().default_provider {
            DefaultProvider::InMemory => {
                (ProviderKind::InMemory, loglet_id.next().to_string().into())
            }
            DefaultProvider::Local => (ProviderKind::Local, loglet_id.next().to_string().into()),
            #[cfg(feature = "replicated-loglet")]
