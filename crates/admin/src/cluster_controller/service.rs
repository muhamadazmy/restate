    ...
    async fn on_cluster_cmd(
        ...
        match command {
            ...
            ClusterControllerCommand::SealAndExtend {
                ...
                info!(log_id=%log_id, segment_index=?segment_index, provider=%provider,"Seal and extend chain");
                let result = bifrost_admin
                    .seal_and_extend_chain(log_id, segment_index, min_version, provider, params)
                    .await;
                info!(log_id=%log_id, segment_index=?segment_index, provider=%provider,"Seal and extend chain completed");
                let _ = response_tx.send(result.map_err(Into::into));
            }
        }
    }
