...
impl<T: TransportConnect> Service<T> {
    ...
    pub async fn run(mut self) -> anyhow::Result<()> {
        ...
        let mut is_active_controller = false;
        ...
        loop {
            tokio::select! {
                ...
                Ok(_) = logs_watcher.changed() => {
                    ...
                    scheduler.on_logs_update(logs.live_load(), partition_table.live_load(), is_active_controller).await?
                }
                Ok(_) = partition_table_watcher.changed()=> {
                    ...
                    scheduler.on_logs_update(logs, partition_table, is_active_controller).await?;
                }
