...
impl<T: TransportConnect> Service<T> {
    ...
    pub async fn run(
        ...
        let mut find_logs_tails_interval =
            time::interval(configuration.admin.log_tail_update_interval.into());
        find_logs_tails_interval.set_missed_tick_behavior(MissedTickBehavior::Delay);
        ...
        loop {
            tokio::select! {
                ...
                _ = find_logs_tails_interval.tick() => {
                    logs_controller.find_logs_tails();
                }
