...
use restate_types::cluster::cluster_state::{
    AliveNode, ClusterState, DeadNode, NodeState, SchrodingerNode,
};
            ...
            while let Some(Ok((node_id, result))) = join_set.join_next().await {
                match result {
                    ...
                    Err(NetworkError::RemoteVersionMismatch(msg)) => {
                        ...
                        // return an RemoteVersionMismatch error.
                        // In this case we are not sure about the peer state but it's
                        // definitely not dead!
                        // Hence we set it as Schrodinger node. This gives it enough time to update
                        // its metadata, before we know the exact state
                        debug!("Node {node_id} is marked as Schrodinger: {msg}");
                        nodes.insert(
                            node_id.as_plain(),
                            NodeState::Schrodinger(SchrodingerNode {
