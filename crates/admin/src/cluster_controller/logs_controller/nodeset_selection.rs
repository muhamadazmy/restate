    ...
    pub fn select<R: Rng + ?Sized>(
        ...
        let nodeset = match strategy {
            NodeSetSelectionStrategy::StrictFaultTolerantGreedy => {
                ...
                // smaller than minimum we try to extend from the full nodeset
...
fn nodeset_size_range(
    ...
    let (nodeset_minimum_size, nodeset_target_size) = match strategy {
        ...
        // in the greedy strategy we take the max(optimal, writable_nodes_size) as
        // our target size
        NodeSetSelectionStrategy::StrictFaultTolerantGreedy => (
            optimal_fault_tolerant_nodeset_size,
            max(optimal_fault_tolerant_nodeset_size, writable_nodes_size),
        ),
    };

    NodeSetSizeRange {
        minimum_size: min_copies.into(),
        fault_tolerant_size: nodeset_minimum_size,
...
pub mod tests {
    ...
    fn test_select_log_servers_insufficient_fault_tolerant_capacity() {
        ...
        let alive_nodes: Vec<PlainNodeId> = vec![1.into(), 2.into()];
        let dead_nodes: Vec<PlainNodeId> = vec![3.into()];
        let replication =
            ReplicationProperty::with_scope(LocationScope::Node, 2.try_into().unwrap());
