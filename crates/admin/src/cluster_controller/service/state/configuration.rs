...
use restate_core::{
    metadata_store::retry_on_network_error, task_center, MetadataWriter, ShutdownError, TaskCenter,
    TaskHandle, TaskKind,
};
...
impl ClusterConfigurationManager {
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<Option<Arc<ClusterConfiguration>>> {
        ...
        let cluster_config: Option<ClusterConfiguration> = if !configuration.common.allow_bootstrap
        {
            ...
            let configuration = retry_on_network_error(
                configuration.common.network_error_retry_policy.clone(),
                || {
                    self.metadata_store_client
                        .get_or_insert(CLUSTER_CONFIG_KEY.clone(), || {
                            ...
                            trace!(
                                ...
                                configuration.common.cluster_configuration_seed
                            );
    ...
    pub fn schedule_refresh(&mut self) -> Result<(), ShutdownError> {
        ...
        let task_handle = self.task_center.spawn_unmanaged(
            TaskKind::Disposable,
