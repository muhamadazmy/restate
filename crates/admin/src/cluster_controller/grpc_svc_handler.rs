...
impl ClusterCtrlSvc for ClusterCtrlSvcHandler {
    ...
    async fn set_cluster_configuration(
        ...
        let partition_replication = match partition_replication_kind {
            PartitionReplicationKind::Limited => Some(PartitionReplication::Limit(
            ...
            PartitionReplicationKind::Unknown => {
                ...
                // if partition_replication is set, then we apply that one as limited
                // otherwise, no changes are applied to current value
