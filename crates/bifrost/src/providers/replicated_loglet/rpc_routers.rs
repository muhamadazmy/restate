...
impl LazyRenewableAppendStream {
    ...
    async fn get<T: TransportConnect>(
        ...
        let connection = networking
            ...
            .await?;
        let stream = RemoteSequencerStream::start(self.append.clone(), connection)?;
...
mod test {
    ...
    use super::RemoteSequencerStream;
    use rand::Rng;
    use restate_core::{
    ...
    async fn test_remote_stream_success() {
        ...
        setup(handler, |test_env| async move {
            ...
            let stream = RemoteSequencerStream::start(test_env.rpc.clone(), connection).unwrap();
