...
pub struct SequencersRpc {
    connection_manager: RemoteSequencerConnectionManager,
...
impl SequencersRpc {
    ...
    pub fn new(router_builder: &mut MessageRouterBuilder) -> Self {
        ...
        Self {
            connection_manager: RemoteSequencerConnectionManager::new(rpc_router),
    ...
    pub async fn append<T: TransportConnect>(
        ...
        let mut connection = self.connection_manager.get(loglet.networking()).await?;
                ...
                Err(err) => {
                    match err {
                        ...
                        | NetworkError::Timeout(_) => {
                            ...
                            connection = self
                                .connection_manager
                                .renew(loglet.networking(), &connection)
                                .await?;
                        }
...
#[derive(Clone)]
struct RemoteSequencerConnectionManager {
    append: RpcRouter<Append>,
    connection: Arc<Mutex<Option<RemoteSequencerConnection>>>,
}

impl RemoteSequencerConnectionManager {
