...
use super::{
    node::{LogServer, LogServerManager},
...
pub(crate) struct Appender {
    ...
    log_server_manager: LogServerManager,
...
impl Appender {
    ...
    async fn send_wave(&mut self, spread: Vec<LogServer>) -> AppenderState {
        ...
        for server in spread {
            ...
            // and just skip
            if server.state().local_tail() > self.first_offset {
                continue;
            }
...
struct Sender {
    global: Arc<SequencerGlobalState>,
    server_manager: LogServerManager,
    server: LogServer,
    first_offset: LogletOffset,
    records: Arc<[Record]>,
...
impl Sender {
    async fn send(mut self) -> (LogServer, Result<Status, NetworkError>) {
        tokio::select! {
            _ = self.global.wait_for_committed_tail(self.first_offset) => {
                // we reached global tail and we can actually commit
            },
            Some(offset) = self.server.state().wait_for_tail(self.first_offset) => {
        ...
        match incoming.status {
            Status::Ok => {
                self.server
                    .state()
                    .maybe_update_local_tail(incoming.local_tail);
            }
            _ => {}
        }
