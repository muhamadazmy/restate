...
use crate::{
    ...
    providers::replicated_loglet::replication::spread_selector::{
        SpreadSelector, SpreadSelectorError,
    },
};
    ...
    pub async fn run(mut self) {
        ...
        loop {
            state = match state {
                ...
                AppenderState::Failure { cause } => {
                    tracing::error!(batch = %self.first_offset, "impossible to satisfy batch replication property: {}", cause);
                    // question: resolve as sealed?
                    self.resolver.sealed();
                    break;
                }
            }
        }
        ...
        let spread = match self.selector.select(
            ...
            Err(err) => {
                if gray_list.is_empty() {
                    ...
                    // no reason to retry.
                    return AppenderState::Failure { cause: err };
                }
