...
impl Payload {
    pub fn inflight_tail(&self) -> Option<LogletOffset> {
        let len = u32::try_from(self.first_offset).ok()?;
...
/// shared status between the appender main loop and
/// the handle
#[derive(Debug)]
struct NodeWorkerStatusInner {
    in_flight: LogletOffset,
    length: usize,
}

#[derive(Clone, Debug)]
pub(crate) struct NodeWorkerHandle {
    inner: Arc<NodeWorkerStatusInner>,
    batch_sender: mpsc::Sender<Weak<Payload>>,
}

impl NodeWorkerHandle {
    pub fn status(&self) -> &NodeWorkerStatusInner {
        &self.inner
    }

    pub async fn reserve(&self) -> Result<SendPermit, mpsc::error::SendError<()>> {
        self.batch_sender.reserve().await
    }
}
    ...
    C: NodeClient + Send + Sync + 'static,
{
    pub fn start(
        ...
        let inner = Arc::new(NodeWorkerStatusInner {
            in_flight: LogletOffset::OLDEST,
            length: 0,
        });
        ...
        tc.spawn_unmanaged(TaskKind::Disposable, "appender", None, appander.run())?;
