...
/// LogServer represents an active log server node.
#[derive(Clone)]
pub struct LogServer {
    node: PlainNodeId,
    state: LogServerState,
    writer: ConnectionSender,
}
...
impl LogServer {
    ...
    pub fn writer(&self) -> ConnectionSender {
        self.writer.clone()
    }
}
...
impl LogServerManager {
    ...
    pub async fn renew(&self, server: &mut LogServer) -> Result<(), Error> {
        ...
        if inner.writer != server.writer {
            // someone else has already renewed the connection
            server.writer = inner.writer.clone();
            return Ok(());
        }

        let sender = self.connect(server.node).await?;
        inner.writer = sender.clone();
        server.writer = sender.clone();
