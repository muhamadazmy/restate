...
use super::{
    writer::{LogServerHandle, LogServerWriter, Payload, SendPermit},
...
pub struct ActiveLogServers {
    inner: BTreeMap<PlainNodeId, LogletNode>,
}

impl ActiveLogServers {
        ...
        C: NodeClient + Send + Sync + 'static,
    {
        ...
        for id in node_set {
            ...
            let worker = LogServerWriter::start(tc, node, 10, Arc::clone(&shared))?;
