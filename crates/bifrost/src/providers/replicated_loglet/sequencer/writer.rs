...
pub(crate) struct LogServerWriter<C> {
    ...
    C: NodeClient + Send + Sync + 'static,
{
    pub fn start(
        ...
        queue_size: usize,
        global: Arc<SequencerGlobalState>,
    ) -> Result<LogServerHandle, ShutdownError> {
        ...
        let handle = LogServerHandle { batch_tx, event_tx };

        let buffer = VecDeque::with_capacity(queue_size);
        let worker = LogServerWriter {
            batch_rx,
            event_rx,
            node,
            global,
            buffer,
            state: LogServerState::Clear,
        };
    ...
    async fn worker_clear(&mut self, token: &CancellationToken) -> LogServerState {
        loop {
            tokio::select! {
                biased;
                _ = token.cancelled() => {
                    return LogServerState::Shutdown;
                }
            ...
            if self.buffer.len() >= self.buffer.capacity() {
                return LogServerState::Congested;
            }
        }
    }

    async fn worker_congested(&mut self, token: &CancellationToken) -> LogServerState {
    ...
    async fn run(mut self) {
        ...
        loop {
            self.state = match self.state {
                LogServerState::Clear => self.worker_clear(&token).await,
                LogServerState::Congested => self.worker_congested(&token).await,
                LogServerState::Shutdown => return,
            }
        }
    }
