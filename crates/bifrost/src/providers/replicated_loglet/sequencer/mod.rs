...
use futures::Stream;
use node::{LogletStatus, NodeClient, NodeSet, ReplicationPolicy, Tracker};
...
impl Sequencer {
    ...
    where
        ...
        P: ReplicationPolicy + Send + Sync + 'static,
    {
...
mod test {
    ...
    use super::{
        node::{NodeClient, PolicyAll, PolicySimpleQuorum, ReplicationPolicy},
    ...
    async fn test_simple_all_replication() {
        let handle = default_setup(2, PolicyAll).await;
    ...
    async fn test_simple_quorum_replication() {
        let handle = default_setup(3, PolicySimpleQuorum).await;
