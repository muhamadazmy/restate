...
use restate_types::{
    ...
    net::log_server::{LogletInfo, Status, Store, StoreFlags, Stored},
    ...
    C: NodeClient + Send + Sync + 'static,
{
    ...
    fn signal_stored(&mut self, stored: Signal<Stored>) {
        if let Some(node) = self.nodes.get(&stored.peer) {
            node.status().touch();
        }
        ...
        for batch in self.batches.iter_mut() {
            // if the stored local tail is less that the batch inflight tail then
            ...
            if stored.local_tail < batch.payload.inflight_tail().unwrap() {
                break;
            }
    ...
    async fn test_setup() {
        ...
        loop {
            ...
            for state in state.values() {
                if state.local_tail == LogletOffset::INVALID {
