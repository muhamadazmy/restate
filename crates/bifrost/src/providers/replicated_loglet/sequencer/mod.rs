...
use futures::Stream;
use node::{LogletStatus, NodeClient, NodeSet};
...
pub use node::{Nodes, ReplicationPolicy};
const SUBSCRIPTION_STREAM_SIZE: usize = 64;
const NODE_HEALTH_CHECK: Duration = Duration::from_millis(250);
...
enum Commands {
    ClusterState(Command<(), ClusterState>),
    /// executed commands
    ...
    C: NodeClient + Send + Sync + 'static,
{
    ...
    fn process_signal(&mut self, signal: Signals) {
        ...
        match signal {
            Signals::Stored(signal) => {
                self.process_stored_signal(signal);
            }
            Signals::LogletInfo(signal) => {
                self.process_loglet_info_signal(signal);
            }
        }
    }
    ...
    async fn enqueue_batch(&mut self, records: Arc<[Record]>) -> Result<LogletCommit, Error> {
        ...
        for id in tracker.node_set.keys() {
    ...
    fn process_stored_signal(&mut self, stored: Signal<Stored>) {
        match stored.status {
    ...
    fn process_loglet_info_signal(&mut self, signal: Signal<LogletInfo>) {
        let Signal {
            peer,
            signal: loglet_info,
        } = signal;
...
mod test {
    ...
    use super::{node::NodeClient, ReplicationPolicy, SequencerHandle, SequencerInner};
    ...
    async fn default_setup(size: usize, policy: ReplicationPolicy) -> SequencerHandle {
    ...
    async fn test_simple_all_replication() {
        let handle = default_setup(2, ReplicationPolicy::All).await;
    ...
    async fn test_simple_quorum_replication() {
        let handle = default_setup(3, ReplicationPolicy::Quorum).await;
