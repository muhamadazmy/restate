...
impl Sequencer {
    pub fn start<T: TransportConnect>(
        node_id: GenerationalNodeId,
        loglet_id: ReplicatedLogletId,
        node_set: NodeSet,
        selector: SpreadSelector,
        ...
        let (handle, sink) = SequencerHandle::pair(Arc::clone(&sequencer_shared_state));
...
impl<T: TransportConnect> SequencerInner<T> {
