...
use node::{ActiveLogServers, LogletStatus, SpreadSelect, Tracker};
use tokio::sync::{mpsc, oneshot};
use writer::{Payload, WorkerEvent};

use restate_core::{
    cancellation_token, cancellation_watcher, network::NetworkError, ShutdownError, TaskCenter,
    TaskKind,
};
    ...
    P: SpreadSelect,
{
    ...
    fn new(
        ...
        // build the node set
        let node_set = ActiveLogServers::start(task_center, node_set, &nodes, Arc::clone(&shared))?;
