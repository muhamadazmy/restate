    ...
    async fn get_loglet<T: TransportConnect>(
        ...
        let loglet = match provider.get_active_loglet(header.log_id, header.segment_index) {
            ...
            None => {
                ...
                let logs_version = peer_version.logs.unwrap_or(current_logs_version);

                // if logs version is still in the future
                // sync to this version (or newer)
                while logs_version > current_logs_version {
                    match provider
                        .networking()
                        .metadata()
                        ...
                        .await
                    {
                        ...
                        Err(SyncError::MetadataStore(err)) => {
                            tracing::trace!(error=%err, target_version=%logs_version, "Failed to sync metadata");
                            //todo(azmy): make sleep it configurable
