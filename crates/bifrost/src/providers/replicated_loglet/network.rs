...
impl RequestPump {
    ...
    async fn get_loglet<T: TransportConnect>(
        ...
        let request_logs_version = peer_version
            .logs
            .unwrap_or(provider.networking().metadata().logs_version());
        ...
        loop {
            ...
            if request_logs_version < provider.networking().metadata().logs_version() {
                match provider
                    .networking()
                    .metadata()
                    .sync(
