    ...
    T: TransportConnect,
{
    ...
    pub fn new(
        ...
        Self {
            ...
            record_permits,
            rpc_router: sequencer_rpc.append,
            known_global_tail,
    ...
    async fn get_connection(&self) -> Result<RemoteSequencerConnection, NetworkError> {
        ...
        let connection = self
            ...
            .await?;
        let stream = RemoteSequencerConnection::start(self.known_global_tail.clone(), connection)?;
        ...
        let reason = loop {
            ...
            tokio::select! {
                appended = inflight.rpc_token.recv() => {
                    ...
                    if let Err(err) = Self::resolve(&known_global_tail, inflight.commit_resolver, appended.into_body()) {
