...
impl RemoteSequencerConnection {
    ...
    async fn handle_appended_responses(
        ...
        let reason = loop {
            let inflight = tokio::select! {
                inflight = rx.recv() => match inflight {
            ...
            tokio::select! {
                // wait for Appended message.
                received = Self::receive_appended(rpc_token, &known_global_tail) => {
                    match received {
                        Ok(offset) => {
                            commit_resolver.offset(offset);
                        },
                        Err(err) => {
                            commit_resolver.error(err.original.clone());

                            if err.terminal {
                                break BreakReason::AppendError(err.original);
                            }
                        }
                    };
                },
                _ = &mut closed => {
                    commit_resolver.error(AppendError::retryable(NetworkError::ConnectionClosed));
                    break BreakReason::ConnectionClosed;
                }
            }
        };
    ...
    /// Wait for [`Appended`] response from leader sequencer.
    ///
    /// The returned error can be terminal error which means
    /// the entire connection is no longer valid and hence
    /// all `pending` inflight appends should be resolved with
    /// a proper [`AppendError`]
    async fn receive_appended(
        rpc_token: RpcToken<Appended>,
        known_global_tail: &TailOffsetWatch,
    ) -> Result<LogletOffset, MaybeTerminalAppendError> {
        let appended = match rpc_token.recv().await {
            Ok(appended) => appended,
            Err(err) => {
                return Err(MaybeTerminalAppendError::terminal(AppendError::Shutdown(
                    err,
                )));
            }
        }
        ...
        match appended.status {
            SequencerStatus::Ok => Ok(appended.first_offset),
            SequencerStatus::Sealed => {
                // A sealed status returns a terminal error since we can immediately cancel
                // all inflight append jobs.
                Err(MaybeTerminalAppendError::terminal(AppendError::Sealed))
            }
            SequencerStatus::Malformed => {
                ...
                Err(MaybeTerminalAppendError::non_terminal(
                    AppendError::terminal(Malformed),
                ))
            }
        }
    }
