...
struct RemoteSequencerConnection {
    ...
    tx: mpsc::Sender<RemoteInflightAppend>,
}
...
impl RemoteSequencerConnection {
    ...
    pub async fn send(
        ...
        let send_permit = self.tx.clone().reserve_owned().await.map_err(|_| {
        ...
        send_permit.send(RemoteInflightAppend {
    ...
    async fn handle_appended_responses(
        known_global_tail: TailOffsetWatch,
        connection: WeakConnection,
        mut rx: mpsc::Receiver<RemoteInflightAppend>,
    ) -> anyhow::Result<()> {
    ...
    async fn handler_loop(
        ...
        rx: &mut mpsc::Receiver<RemoteInflightAppend>,
    ) -> AppendError {
