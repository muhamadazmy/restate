...
impl RemoteSequencerConnection {
    ...
    async fn handle_appended_response(
        ...
        let inflight = rx
            .recv()
            .await
            .ok_or(AppendError::Shutdown(ShutdownError))?;
        ...
        let appended = rpc_token.recv().await?.into_body();
        ...
        let reason = loop {
            ...
            match appended.status {
                ...
                SequencerStatus::Sealed => {
                    ...
                    // all inflight append jobs.
                    commit_resolver.sealed();
                    break BreakReason::AppendError(AppendError::Sealed);
                }
            }
        };
