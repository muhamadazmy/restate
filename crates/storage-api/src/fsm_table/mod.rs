...
impl Merge for CachedEpochMetadata {
    fn merge(&mut self, other: Self) -> bool {
        ...
        if other.leader_epoch > self.leader_epoch {
            self.leader_epoch = other.leader_epoch;
            modified = true;
        }

        if other.leader_node_id != self.leader_node_id {
            self.leader_node_id = other.leader_node_id;
            modified = true;
        }

        if other.current.replica_set.version > self.current.replica_set.version {
            self.current = other.current;
            modified = true;
        }

        match (other.next, self.next.as_ref()) {
            (value, None) => {
                modified = value.is_some();
                self.next = value;
            }
            (Some(o), Some(s)) => {
                if o.replica_set.version > s.replica_set.version {
                    self.next = Some(o);
                    modified = true;
                }
            }
