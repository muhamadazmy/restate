...
use restate_types::{
    Version,
    config::{DynamoDbOptions, MetadataClientOptions},
    metadata::{Precondition, VersionedValue},
    net::connect_opts::CommonClientConnectionOptions,
};
...
const CONDITION_DOES_NOT_EXIST: &str =
    const_format::concatcp!("attribute_not_exists(", KIND_FIELD, ")");
...
impl DynamoDbMetadataStore {
    pub async fn new(
        ...
        // todo: figure out proper configuration.
        let cfg = aws_config::defaults(BehaviorVersion::latest()).timeout_config(timeout_config);
        ...
        let cfg = match (
            ...
            (Some(_), Some(_)) => anyhow::bail!(
                "Either `aws-profile` or `aws-access-key-id` must be provided for dynamodb metadata provider, but not both"
            ),
            ...
            (None, Some(access_key_id)) => {
                let secret_access_key =
                    dynamo_db_options.aws_secret_access_key.ok_or_else(|| {
                        anyhow::anyhow!(
                            "Missing aws-secret-access-key for dynamodb metadata provider"
                        )
                    })?;
...
impl ProvisionedMetadataStore for DynamoDbMetadataStore {
    ...
    async fn get(&self, key: ByteString) -> Result<Option<VersionedValue>, ReadError> {
        let result = self
            ...
            .key(KIND_FIELD, AttributeValue::S(key.to_string()))
            .send()
            .await
    ...
    async fn put(
        ...
        let put = self
            ...
            .item(KIND_FIELD, AttributeValue::S(key.to_string()))
            .item(
                VERSION_FIELD,
...
mod test {
    ...
    // todo(azmy): right now the test relies on a running dynamodb instance in the test environment.
    // which is not available atm.
    // this is why all tests below are [ignored] for now and should be enabled only to run manually
    // in a development env
