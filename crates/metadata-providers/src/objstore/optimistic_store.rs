...
fn tagged_value_to_versioned_value(
    ...
    let versioned_value = match tagged_value.encoding {
        ...
        ValueEncoding::Bilrost => SaltedVersionedValue::decode(tagged_value.bytes.as_ref())
...
impl OptimisticLockingMetadataStore {
    const DEFAULT_ENCODING: ValueEncoding = ValueEncoding::Cbor;
    ...
    pub(crate) async fn put(
        ...
        // The salt is a random value to break CAS ties.
        // S3 cas working by comparing ETags (hash of the body) and we don't want a case where two writer think they both succeeded the cas (they both CASed the same body)
        let salt = random::<u64>();

        let content = self.serialize_versioned_value(Self::DEFAULT_ENCODING, value, salt)?;
        match precondition {
            Precondition::None => {
