...
impl PartitionTableBuilder {
        ...
        F: FnMut(&PartitionId, &mut TargetPartitionReplicationState),
    {
        for (partition_id, partition) in self.inner.partitions.iter_mut() {
            ...
            // auto detect change in replica group either by
            // change in length or if items at same index are not
            // equal anymore.
            // we can't do `left != right` since IndexSet(s) are equal
            // if they have the same items regardless of the order.
            if partition.replica_group.len() != replica_group.len()
                || partition
                    .replica_group
                    .iter()
                    .zip(replica_group.iter())
                    .any(|(l, r)| *l.0 != *r.0)
            {
                self.modified = true;
