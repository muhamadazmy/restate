...
use crate::{
    cluster::cluster_state::PartitionProcessorStatus, identifiers::PartitionId,
...
mod test {
    ...
    fn encoding() {
        ...
        let result = flexbuffers::to_vec(req);
        assert!(result.is_ok());

        let req: Result<GossipPayload, _> = flexbuffers::from_slice(&result.unwrap());
        assert!(req.is_ok());
    }
}
