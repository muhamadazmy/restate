...
impl WireEncode for Store {
    fn encode_to_bytes(
        ...
        } else if protocol_version == ProtocolVersion::V2 {
...
impl WireDecode for Store {
        ...
        Self: Sized,
    {
        if protocol_version < ProtocolVersion::V2 {
            ...
            let msg: compat::Store = crate::net::codec::decode_as_bilrost(buf, protocol_version)?;
            Ok(msg.into())
        } else {
...
mod compat {
    ...
    /// Unlike the newer [`Payloads`] which uses custom encoding to generate
