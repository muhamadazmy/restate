...
impl Subscription {
    ...
    pub fn from_cluster(
        ...
        let mut merged = cluster.additional_options.clone();
        ...
        if merged.contains_key("enable.auto.commit") {
            warn!(
                "The configuration option enable.auto.commit should not be set and it will be ignored."
            );
        }

        if merged.contains_key("enable.auto.offset.store") {
            warn!(
                "The configuration option enable.auto.offset.store should not be set and it will be ignored."
            );
        }

        if !merged.contains_key("group.id") {
            merged.insert("group.id".into(), Ulid::new().to_string());
        }

        // Set client.id if unset
        if !merged.contains_key("client.id") {
            merged.insert("client.id".into(), "restate".to_string());
        }

        Ok(Self::new(source, sink, merged))
    }
