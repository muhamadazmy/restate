...
impl Display for Rate {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Rate::PerSecond(rate) => write!(f, "{}/sec", rate),
            Rate::PerMinute(rate) => write!(f, "{}/min", rate),
            Rate::PerHour(rate) => write!(f, "{}/hr", rate),
        }
    }
}
