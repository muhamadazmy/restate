...
impl UnresolvedFuture {
    pub fn split(
        ...
        (self.combinator, self.notifications, self.nested)
    }
...
impl FromIterator<NotificationId> for UnresolvedFuture {
    fn from_iter<T: IntoIterator<Item = NotificationId>>(iter: T) -> Self {
        Self {
            notifications: iter.into_iter().collect(),
            nested: Vec::default(),
            combinator: CombinatorType::Unknown,
        }
    }
