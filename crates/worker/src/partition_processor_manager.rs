...
impl<T: TransportConnect> PartitionProcessorManager<T> {
    ...
    pub async fn run(mut self) -> anyhow::Result<()> {
        ...
        self.apply_plan(&msg.actions, event_tx.clone()).await?;
        ...
        loop {
            tokio::select! {
                ...
                Some(control_processors) = self.incoming_update_processors.next() => {
                    self.on_control_processors(control_processors, effect_tx.clone())
                }
                Some(event) = event_rx.recv() => {
                    self.on_processor_event(event);
                }
                Some(effect) = effect_rx.recv() => {
                    // to easily follow the effect from the origin
                    // the span is created as a child of effect.parent.
                    let span = debug_span!(parent: &effect.parent, "processor_manager_effect");
                    span.follows_from(Span::current());
                    if let Err(err) = self.on_processor_effect(effect.effect, event_tx.clone()).instrument(span).await {
                        warn!("failed processing control processors effects: {err}");
                    }
                }
                _ = &mut shutdown => {
    ...
    async fn on_processor_effect(
        &mut self,
        effect: ProcessorEffect,
        events: mpsc::UnboundedSender<ManagerEvent>,
    ) -> Result<(), Error> {
        match effect {
            ProcessorEffect::ControlProcessor {
                control_processor,
                partition_table,
            } => {
                self.on_control_processor(control_processor, &partition_table, events)
                    .await
            }
        }
    }

    fn on_processor_event(&mut self, event: ManagerEvent) {
        let ManagerEvent {
            partition_id,
            event,
        } = event;
    ...
    fn on_control_processors(
        ...
        let handle_control_processors = async move {
            metadata
                .wait_for_version(
                    MetadataKind::Logs,
        ...
        let _ = self.task_center.spawn_child(
            TaskKind::Disposable,
            "on_control_processors",
            None,
            handle_control_processors
                .instrument(debug_span!(parent: None, "on_control_processors")),
        );
    }
...
struct ManagerEffect {
    parent: Span,
    effect: ProcessorEffect,
}

enum ProcessorEffect {
    ControlProcessor {
        control_processor: ControlProcessor,
...
impl<T: TransportConnect> SpawnPartitionProcessorTask<T> {
    ...
    async fn run(self) -> anyhow::Result<()> {
        ...
        let _ = self.events.send(ManagerEvent {
            partition_id: self.partition_id,
