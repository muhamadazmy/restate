...
pub struct PartitionProcessorManager<T> {
    ...
    control_processors: Option<ControlProcessors>,
}
...
impl<T: TransportConnect> PartitionProcessorManager<T> {
    ...
    pub async fn run(mut self) -> anyhow::Result<()> {
        ...
        let (processor_mgr_effect_tx, mut processor_mgr_effect_rx) = mpsc::unbounded_channel();
            ...
            tokio::select! {
                ...
                Some(control_processors) = self.incoming_update_processors.next() => {
                    self.on_control_processors(
                        control_processors.into_body(),
                        processor_mgr_effect_tx.clone())?;
                }
                _ = logs_version_watcher.changed(), if self.control_processors.is_some() => {
                    ...
                    let control_processors = self.control_processors.take().unwrap();
                    self.on_control_processors(
                        control_processors,
                        processor_mgr_effect_tx.clone())?;
                }
                ...
                Some(effect) = processor_mgr_effect_rx.recv() => {
                    ...
                    if let Err(err) = self.on_processor_mgr_effect(effect.effect, processor_mgr_event_tx.clone()).instrument(span).await {
    ...
    async fn on_processor_mgr_effect(
        &mut self,
    ...
    fn on_control_processors(
                ...
                <= self.metadata.partition_table_version()
        {
            ...
            self.control_processors = Some(control_processors);
        }
