...
const CLEANER_EFFECT_BATCH_SIZE: usize = 250;
...
impl CleanerHandle {
    ...
    pub fn effects(&mut self) -> impl Stream<Item = Vec<CleanerEffect>> {
        (&mut self.rx).ready_chunks(CLEANER_EFFECT_BATCH_SIZE)
    }
}
    ...
    Storage: ScanInvocationStatusTable + Send + Sync + 'static,
{
    ...
    pub(super) fn start(self) -> Result<CleanerHandle, ShutdownError> {
        let (tx, rx) = mpsc::channel(CLEANER_EFFECT_BATCH_SIZE);
...
mod tests {
    ...
    pub async fn cleanup_works() {
        ...
        let received: Vec<_> = handle.effects().next().await.unwrap();
