...
use restate_storage_api::fsm_table::{
    PartitionDurability, ReadFsmTable, StoredPartitionReplicaSetState, WriteFsmTable,
};
...
pub struct LeadershipInfo {
    ...
    pub current_config: ReplicaSetState,
    pub next_config: Option<ReplicaSetState>,
}
    ...
    InvokerSender: restate_invoker_api::InvokerHandle<InvokerStorageReader<PartitionStore>> + Clone,
{
    ...
    fn update_observed_membership(
        ...
        if let Some(current_config) = &announce_leader.current_config {
            ...
            let last_observed = self.replica_set_states.membership_state(partition_id);

            // Persist partition configuration state if present (since v1.6)
            transaction.put_partition_config_state(&StoredPartitionReplicaSetState {
                current: last_observed.observed_current_membership,
                next: last_observed.observed_next_membership,
            })?;
        } else {
    ...
    async fn on_target_leader_state(
        ...
        match target_leader_state {
            TargetLeaderState::Leader(mut leadership_info) => {
                self.status.planned_mode = RunMode::Leader;

                self.replica_set_states.note_observed_membership(
                    self.partition_store.partition_id(),
                    restate_types::partitions::state::LeadershipState {
                        current_leader_epoch: leadership_info.leader_epoch,
                        current_leader: Metadata::current().my_node_id(),
                    },
                    &leadership_info.current_config,
                    &leadership_info.next_config,
                );
                ...
                let last_known_epoch = latest_observed.current_leader().current_leader_epoch;
                leadership_info.current_config = latest_observed.observed_current_membership;
                leadership_info.next_config = latest_observed.observed_next_membership;

                match last_known_epoch.cmp(&leadership_info.leader_epoch) {
                    ...
                    Ordering::Greater => {
                        // newer leader has been observed.
