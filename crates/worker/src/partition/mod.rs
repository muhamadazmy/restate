    ...
    InvokerSender: restate_invoker_api::InvokerHandle<InvokerStorageReader<PartitionStore>> + Clone,
{
    ...
    async fn run_inner(&mut self) -> Result<(), ProcessorError> {
        ...
        let apply_command_latency =
            histogram!(PARTITION_APPLY_COMMAND_DURATION, PARTITION_LABEL => partition_id_str);
        let record_actions_latency = histogram!(PARTITION_LEADER_HANDLE_ACTION_BATCH_DURATION);
        let record_write_to_read_latencty = histogram!(PARTITION_RECORD_WRITE_TO_READ_LATENCY_SECONDS, PARTITION_LABEL => partition_id_str);
        let command_batch_size =
            histogram!(PARTITION_APPLY_COMMAND_BATCH_SIZE, PARTITION_LABEL => partition_id_str);
