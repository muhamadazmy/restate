    ...
    T: TransportConnect,
{
    ...
    async fn announce_leadership(&mut self, leadership_info: LeadershipInfo) -> Result<(), Error> {
        ...
        let announce_leader = Command::AnnounceLeader(Box::new(AnnounceLeader {
            ...
            current_config: Some(leadership_info.current_config.into()),
            next_config: leadership_info.next_config.map(Into::into),
        }));
...
mod tests {
    ...
    async fn become_leader_then_step_down() -> googletest::Result<()> {
        ...
        let leadership_info = LeadershipInfo {
            leader_epoch,
            current_config: current_config.to_replica_set_state(),
            next_config: None,
        };
