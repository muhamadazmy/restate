...
use restate_types::journal_v2::{Entry, NotificationsCombinator};
...
pub fn invoker_suspended(
    invocation_id: InvocationId,
    combinator: impl Into<NotificationsCombinator>,
) -> Command {
    let combinator = combinator.into();
    Command::InvokerEffect(Box::new(Effect {
        invocation_id,
        kind: InvokerEffectKind::SuspendedV2 {
            waiting_for_notifications: combinator.flatten(),
