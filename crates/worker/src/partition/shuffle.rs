...
mod shuffle_stream {
    ...
    use restate_ingestion_client::{CommitError, IngestFuture, IngestionClient, RecordCommit};
    use restate_storage_api::outbox_table::OutboxMessage;
    ...
    struct RecordCommitWithSn {
        inner: RecordCommit,
        sn: u64,
    }

    impl RecordCommitWithSn {
        fn new(inner: RecordCommit, sn: u64) -> Self {
            Self { inner, sn }
        }
    }

    impl Future for RecordCommitWithSn {
        type Output = Result<u64, CommitError>;

        fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
            let result = ready!(self.inner.poll_unpin(cx));
            Poll::Ready(result.map(|_| self.sn))
        }
    }
    ...
    pub struct ShuffleStream<T, R> {
        ...
        inflight: VecDeque<RecordCommitWithSn>,
        next_sequence_number: MessageIndex,
        #[pin]
        state: State,
    }
            ...
            loop {
                match this.state.as_mut().project() {
                    ...
                    StateProj::Ingesting { ingest, sn } => {
                        ...
                        this.inflight
                            .push_back(RecordCommitWithSn::new(commit_token, sn));
