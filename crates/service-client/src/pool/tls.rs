    ...
    S::Future: Send,
{
    ...
    fn call(&mut self, req: Uri) -> Self::Future {
        let connection_info = match req.get_connection_info() {
            Ok(info) => info,
            Err(err) => return TlsConnectorFutureInner::fail(err).into(),
        };

        let mut this = mem::replace(self, self.clone());
        let fut = this.inner.call(req.clone());
        ...
        loop {
            match this.as_mut().project() {
                ...
                TlsConnectorFutureProj::Connecting {
                    ...
                    Poll::Ready(Ok(socket)) => match connection_info.secure() {
                        false => {
                        ...
                        true => {
                            let fut = connector
                                .clone()
                                .connect(connection_info.host().clone().into(), socket);
                            this.set(TlsConnectorFutureInner::Handshaking { fut: Box::new(fut) });
                        }
                    },
                },
                TlsConnectorFutureProj::Handshaking { fut } => match fut.poll(cx) {
                    Poll::Pending => return Poll::Pending,
