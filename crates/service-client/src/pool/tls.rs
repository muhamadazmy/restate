...
/// to the inner stream.
#[pin_project::pin_project(project=MaybeTlsStreamProject)]
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    fn poll_read(
        ...
        match self.project() {
            MaybeTlsStreamProject::Plain(plain) => plain.poll_read(cx, buf),
            MaybeTlsStreamProject::TLS(tls) => tls.poll_read(cx, buf),
        }
    }
}
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    fn poll_write(
        ...
        match self.project() {
            MaybeTlsStreamProject::Plain(plain) => plain.poll_write(cx, buf),
            MaybeTlsStreamProject::TLS(tls) => tls.poll_write(cx, buf),
        }
    }
    ...
    fn poll_flush(
        ...
        match self.project() {
            MaybeTlsStreamProject::Plain(plain) => plain.poll_flush(cx),
            MaybeTlsStreamProject::TLS(tls) => tls.poll_flush(cx),
        }
    }
    ...
    fn poll_shutdown(
        ...
        match self.project() {
            MaybeTlsStreamProject::Plain(plain) => plain.poll_shutdown(cx),
            MaybeTlsStreamProject::TLS(tls) => tls.poll_shutdown(cx),
        }
    }
}
