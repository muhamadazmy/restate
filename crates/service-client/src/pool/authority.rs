...
use super::conn::{Connection, ConnectionError, RequestFuture};
    ...
    C::Error: Into<ConnectionError>,
{
        ...
        loop {
            ...
            if let Some(err) = failure {
                ...
                if !self.candidates.is_empty() {
                    // other candidates might succeed
                    return Poll::Pending;
                }
            ...
            if inner.connections.len() < self.config.max_connections.get() {
                let connection =
                    Connection::new(self.connector.clone(), self.config.init_max_streams.get());
    ...
    pub fn call<B>(&mut self, request: http::Request<B>) -> RequestFuture<B>
    where
...
mod test {
    ...
    fn make_pool(
        ...
        AuthorityPool::new(
            ...
            PoolConfig {
                ...
                init_max_streams: NonZeroU32::new(max_concurrent_streams).unwrap(),
            },
        )
    }
    ...
    async fn clones_share_connections() {
        ...
        {
            let inner = p2.inner.lock().unwrap();
            assert_eq!(inner.connections.len(), 1);
        }
        ...
        {
            let inner = p1.inner.lock().unwrap();
            assert_eq!(inner.connections.len(), 1);
        }
    }
