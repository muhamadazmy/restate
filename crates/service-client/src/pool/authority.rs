    ...
    pub fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<(), Error>> {
        ...
        loop {
            ...
            while !inner.connections.is_empty() && index < inner.connections.len() {
                ...
                index += 1;
                // Todo: Consider skipping candidates with fewer than 20% of max concurrent streams
                // remaining. Which can be a huge waste for connection was high number of concurrent
                // streams.
