    ...
    C::Error: Into<Error>,
{
    ...
    pub fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<(), Error>> {
        ...
        loop {
            ...
            let should_expand_eagerly = if total_max_concurrent_streams == 0 {
                // No live connections — definitely create one
                true
            } else {
                let available_ratio =
                    total_available_streams as f64 / total_max_concurrent_streams as f64;
                available_ratio < 1.0 - self.config.connection_saturation_threshold.clamp(0.0, 1.0)
            };

            if should_expand_eagerly {
