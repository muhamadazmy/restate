    ...
    C::Error: Into<Error>,
{
    ...
    pub fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<(), Error>> {
        ...
        loop {
            ...
            let should_eager_expand = if total_max_concurrent_streams == 0 {
                ...
                available_ratio
                    < self
                        .config
                        .eager_connection_threshold_percent
                        .clamp(0.0, 1.0)
            };

            if should_eager_expand {
            ...
            // No new candidates from the pool. Also no eager expansion (disabled)
            // Try to create a fresh connection if there are closed connections to evict or we're under the limit.
            if let Some(mut candidate) = self.try_new_connection(has_some_closed, &mut inner) {
