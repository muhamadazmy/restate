...
pub struct Pool<C> {
    connector: C,
    config: PoolConfig,
    authorities: Arc<DashMap<ConnectionInfo, AuthorityPool<C>>>,
}
    ...
    C::Error: Into<ConnectionError>,
{
    ...
    ) -> Result<Response<PermittedRecvStream>, ConnectionError>
    where
        B: Body<Data = Bytes> + Unpin + Send + Sync + 'static,
        B::Error: Into<Box<dyn std::error::Error + Send + Sync>> + Send,
    {
        let connection_info = request.uri().get_connection_info()?;
        let mut inner_pool = self
            .authorities
            .entry(connection_info)
            .or_insert_with(|| AuthorityPool::new(self.connector.clone(), self.config.clone()))
            .value()
            .clone();

        poll_fn(|cx| inner_pool.poll_ready(cx)).await?;
        inner_pool.call(request).await
    }
}
...
mod test {
    ...
    fn make_pool(
        ...
        PoolBuilder::default()
            ...
            .init_max_streams(std::num::NonZeroU32::new(max_concurrent_streams).unwrap())
