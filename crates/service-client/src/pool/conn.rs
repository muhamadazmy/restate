    ...
    C::Error: Into<ConnectionError>,
{
    ...
    pub fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<(), ConnectionError>> {
        match self.inner.lock().unwrap().deref_mut() {
            ...
            ConnectionInner::Connected { send_request } => {
                ...
                let state = RequestFutureState::drive(async move {
                    ...
                    tokio::spawn(async move {
                        if let Err(err) = connection.await {
                            debug!("h2 connection closed: {err}");
                        }
...
enum ConnectionInner {
    ...
    Connected { send_request: SendRequest<Bytes> },
...
/// - **Connecting** – this request is driving the initial H2 handshake.
...
enum RequestFutureState {
    Connecting {
        #[pin]
        fut: BoxFuture<'static, Result<SendRequest<Bytes>, ConnectionError>>,
    },
    WaitingConnection {
        #[pin]
...
impl RequestFutureState {
    ...
    where
        F: Future<Output = Result<SendRequest<Bytes>, ConnectionError>> + Send + 'static,
    {
        Self::Connecting { fut: Box::pin(fut) }
    }
}
...
impl<B> PinnedDrop for RequestFuture<B> {
    fn drop(self: Pin<&mut Self>) {
        // if the `Connecting`` future was dropped. we need to
        // immediately switch connection to closed.
        // to make sure the connection cannot be used any more
        // and notify any potential waiters
        ...
        loop {
            match this.state.as_mut().project() {
                RequestFutureStateProject::Error { err } => {
                    // we make sure to set the state 'closed' to notify any waiters
                    // that the connection is no longer valid
                    *this.connection.lock().unwrap() = ConnectionInner::Closed;
                    return Poll::Ready(Err(err.take().expect("future polled after finish")));
                }
                RequestFutureStateProject::Connecting { fut } => {
                    let send_request = match ready!(fut.poll(cx)) {
                        Ok(stream) => stream,
                        Err(err) => {
