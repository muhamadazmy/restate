...
pub enum Error<R> {
    ...
    #[error("permit to use the connection was lost")]
    PermitReclaimed(R),
}
    ...
    C::Error: Into<ConnectionError>,
{
    pub fn new(connector: C, config: ConnectionConfig) -> Self {
        Self {
            connector,
            concurrency: Concurrency::new(config.initial_max_send_streams as usize),
    ...
    pub fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<(), ConnectionError>> {
        ...
        if self.acquire.is_none() {
            self.acquire = Some(Box::pin(self.concurrency.acquire()));
        }
    ...
    fn handle_state(&self, mut state: u8) -> ResponseFutureState {
        loop {
            match state {
                ...
                STATE_CLOSED => return ResponseFutureState::error(ConnectionError::Closed),
                _ => unreachable!(),
            }
        }
    }
    ...
    fn drive_handshake<B>(&mut self, request: &http::Request<B>) -> ResponseFutureState {
        ...
        ResponseFutureState::drive(async move {
            ...
            tokio::task::Builder::new()
                .name("H2::Pool(connection-event-loop)")
                .spawn(async move {
...
{
    type Output = Result<Response<PermittedRecvStream>, Error<http::Request<B>>>;
            ...
            match this.state {
                ...
                ResponseFutureState::WaitingConnection { ref mut rx } => {
                    match ready!(rx.poll_unpin(cx)) {
                        ...
                        Err(_) => {
                            this.state = ResponseFutureState::error(ConnectionError::Closed);
                            continue;
                        }
                    }
                }
                        ...
                        Poll::Pending => {
                            ...
                            match permit.poll_reclaimed(cx) {
                                Poll::Ready(()) => {
                                    ...
                                    return Poll::Ready(Err(Error::PermitReclaimed(
                ...
                ResponseFutureState::InFlight { ref mut fut } => {
                    let resp = ready!(fut.poll_unpin(cx)).map_err(ConnectionError::from)?;
    ...
    B::Error: Into<Box<dyn std::error::Error + Send + Sync>> + Send,
{
    ...
    async fn run(mut self) {
        if let Err(err) = self.run_inner().await {
            self.send_stream
                .send_reset(err.reason().unwrap_or(Reason::INTERNAL_ERROR));
        }
    }
    ...
    async fn run_inner(&mut self) -> Result<(), h2::Error> {
        while let Some(frame) = self.body.frame().await {
            match frame {
                ...
                Err(err) => {
                    debug!("request stream failed with an error: {}", err.into());
                    return Err(Reason::CANCEL.into());
                }
            }
        }
    ...
    async fn waiters_notified_on_driver_drop() {
        ...
        for result in &results {
            ...
            assert!(
                matches!(
                    err,
                    super::Error::ConnectionError(super::ConnectionError::Closed)
                ),
