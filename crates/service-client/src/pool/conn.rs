    ...
    C::Error: Into<ConnectionError>,
{
    ...
    pub fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<(), ConnectionError>> {
        ...
        self.permit = Some(
            ready!(acquire.poll_unpin(cx))
                .expect("Semaphore can never be closed because we already own it"),
        );
        self.acquire = None;
        ...
        loop {
            match this.state {
                ...
                ResponseFutureState::Driving { ref mut fut } => {
                    ...
                    this.shared.state.store(STATE_CONNECTED, Ordering::Relaxed);
                ...
                ResponseFutureState::InFlight { ref mut fut } => {
                    ...
                    let permit = this.permit.take().expect("permit not taken");
...
impl SemaphoreUpdater {
    ...
    fn update(&self, target_max_send_streams: usize, permits: &Semaphore) {
        let target = target_max_send_streams.min(Semaphore::MAX_PERMITS);
...
mod test {
    ...
    impl ControlledConnector {
        fn new(max_concurrent_streams: u32) -> (Self, Arc<tokio::sync::Notify>) {
        ...
        fn with_error(max_concurrent_streams: u32) -> (Self, Arc<tokio::sync::Notify>) {
    ...
    async fn waiters_notified_on_driver_drop() {
        ...
        for result in &results {
            ...
            assert!(
                matches!(err, super::ConnectionError::Closed),
