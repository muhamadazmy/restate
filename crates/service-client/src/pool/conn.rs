...
{
    pub fn new(connector: C, init_max_send_streams: u32) -> Self {
        Self {
            connector,
            semaphore_updater: Arc::new(SemaphoreUpdater::new(init_max_send_streams as usize)),
            semaphore: Arc::new(Semaphore::new(init_max_send_streams as usize)),
    ...
    fn drive_handshake<B>(&mut self, request: &http::Request<B>) -> ResponseFutureState {
        ...
        let max_send_streams = self.semaphore_updater.current();
        ...
        loop {
            match this.state.as_mut().project() {
                ...
                RequestFutureStateProject::WaitingConnection { rx } => match ready!(rx.poll(cx)) {
                    ...
                    Err(_) => {
                        this.state
                            .set(ResponseFutureState::error(ConnectionError::Cancelled));
                        continue;
                    }
                },
