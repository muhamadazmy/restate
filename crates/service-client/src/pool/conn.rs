    ...
    C::Error: Into<ConnectionError>,
{
        ...
        B::Error: Into<Box<dyn std::error::Error + Send + Sync>>,
    {
        ...
        let state = match &mut *inner {
            ConnectionInner::Closed => ResponseFutureState::error(ConnectionError::Closed),
            ConnectionInner::New => {
                let weak_inner = Arc::downgrade(&self.inner);

                let connect = self.connector.call(request.uri().clone());
                let max_streams = self.semaphore_updater.current();
                let state = ResponseFutureState::drive(async move {
            ...
            ConnectionInner::Connected { send_request, .. } => ResponseFutureState::PreFlight {
