...
impl Drop for Permit {
    fn drop(&mut self) {
        ...
        debug_assert!(prev != 0);
        self.inner.capacity.notify_one();
    }
}
            ...
            match this.state.as_mut().project() {
                PermitFutureStateProject::TryAcquire => {
                    ...
                    this.state.set(PermitFutureState::Waiting {
                        notified: this.concurrency.capacity.clone().notified_owned(),
                    });
                    ...
                    let reclaimed = this.concurrency.reclaim.clone().notified_owned();
...
/// Wrapper around [`OwnedNotified`] that is `Unpin` by boxing the inner future,
/// preventing `!Unpin` from propagating through the rest of the type hierarchy.
#[derive(Debug)]
