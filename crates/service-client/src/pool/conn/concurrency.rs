        ...
        loop {
            match this.state.as_mut().project() {
                PermitFutureStateProject::TryAcquire => {
                    ...
                    // it's okay to not enable (reclaimed) unlike notified. because we always use
                    // `notify_waiters()` which works for all waiters created before calling notify_waiters()
                    // even if they aren't enabled/polled before.
