...
impl Concurrency {
    ...
    /// Existing permits beyond the new limit are not revoked; they drain naturally.
    pub fn resize(&self, size: usize) {
        let prev = self.inner.size.swap(size, Ordering::Relaxed);
        if prev < size {
...
impl Drop for Permit {
    fn drop(&mut self) {
        let prev = self.inner.inner.inflight.fetch_sub(1, Ordering::Relaxed);
        debug_assert!(prev != 0);
        ...
        loop {
            match this.state.as_mut().project() {
                PermitFutureStateProject::TryAcquire => {
                    ...
                    let notified = this.concurrency.notify.clone().notified_owned();
                    ...
                    this.state.set(PermitFutureState::Waiting { notified });
                }
...
mod tests {
    ...
    async fn resize_down_drains_existing_permits() {
        ...
        // Now inflight is 1. Drop _p3 (held by the outer scope) would unblock,
        // but let's resize back up instead to verify combined behavior.
        conc.resize(2);
