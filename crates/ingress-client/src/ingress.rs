...
mod test {
    ...
    use crate::{IngressClient, SessionOptions};
    ...
    async fn test_session_single_record() {
    ...
    #[test(restate_core::test)]
    async fn test_session_single_record_retry() {
        let (mut incoming, client) = init_env(10, Duration::ZERO).await;
    ...
    async fn test_session_batching() {
        ...
        let msg = must_next(&mut incoming).await;
        let (rx, body) = msg.split();
        assert_eq!(body.records.len(), 3); //2 messages + marker for the test
        assert_eq!(body.records[0].record, Bytes::from("msg 0"));
        rx.send(IngestResponse::Ack);
        ...
        let msg = must_next(&mut incoming).await;
        let (rx, body) = msg.split();
        assert_eq!(body.records.len(), 10); //9 messages + marker for the test
        assert_eq!(body.records[9].record, Bytes::from("marker"));
        rx.send(IngestResponse::Ack);
