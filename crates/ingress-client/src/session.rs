    ...
    T: TransportConnect,
{
    ...
    pub async fn start(mut self, cancellation: CancellationToken) {
        ...
        loop {
            state = match state {
                SessionState::Connecting { retry } => self.connect(&cancellation, retry).await,
                SessionState::Connected { connection } => {
                    self.connected(&cancellation, connection).await
                }
    ...
    async fn connected(
        ...
        let mut chunked = std::pin::pin!(chunked);
        ...
        // state != Connected
        assert!(!matches!(state, SessionState::Connected { .. }));
