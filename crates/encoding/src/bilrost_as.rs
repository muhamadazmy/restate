...
/// A special adaptor that always ignore the field and decode always as the default
/// value
pub struct BilrostIgnore;

impl<'a, Source> BilrostAsAdaptor<'a, Source> for BilrostIgnore
where
    Source: Default,
{
...
impl Wiretyped<General> for BilrostIgnore {
    const WIRE_TYPE: bilrost::encoding::WireType = <() as Wiretyped<General>>::WIRE_TYPE;
}

impl ValueEncoder<General> for BilrostIgnore {
...
impl ValueDecoder<General> for BilrostIgnore {
    fn decode_value<B: bytes::Buf + ?Sized>(
        _value: &mut Self,
...
impl ForOverwrite for BilrostIgnore {
    fn for_overwrite() -> Self
    where
        Self: Sized,
    {
...
impl EmptyState for BilrostIgnore {
    fn clear(&mut self) {}
    fn empty() -> Self
    where
        Self: Sized,
