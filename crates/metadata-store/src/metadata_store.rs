...
const METADATA_SIZE_SOFT_LIMIT_WARNING: usize = 4 * 1024 * 1024; //4MB
...
impl MetadataStoreClient {
    ...
    fn warn_if_oversized(&self, key: &ByteString, value: &VersionedValue) {
        ...
        if size < METADATA_SIZE_SOFT_LIMIT_WARNING {
            return;
        }
        ...
        match &key[..] {
            Schema::KEY => {
                warn!(
                    "Schema metadata is {size} bytes (soft limit {METADATA_SIZE_SOFT_LIMIT_WARNING}). \
                    Remove unused deployments or services to keep metadata manageable."
                );
            }
            NodesConfiguration::KEY => {
                warn!(
                    "Nodes metadata is {size} bytes (soft limit {METADATA_SIZE_SOFT_LIMIT_WARNING}). \
                    Remove dead nodes to keep metadata manageable."
                );
            }
            _ => {
                warn!(
                    "Metadata entry '{key}' is {size} bytes, above the soft limit of {METADATA_SIZE_SOFT_LIMIT_WARNING} bytes."
                );
            }
        }
    }
