...
where
    T: TransportConnect,
{
    pub fn create(networking: Networking<T>, router_builder: &mut MessageRouterBuilder) -> Self {
        let incoming_node_state = router_builder.subscribe_to_stream(2);
        let cluster_state_refresher = ClusterStateRefresher::new(networking, router_builder);
        Self {
