...
use restate_core::{
    ...
    network::{
        ...
        PeerMetadataVersion, TransportConnect,
    },
    task_center,
    ...
    T: TransportConnect,
{
    ...
    async fn on_gossip_response(
        ...
        // only accept the peer updates if it at equal or higher version as us.
        if self.update_versions(msg.metadata_version()) {
    ...
    async fn on_gossip_request(
        ...
        // only accept the peer updates if it at equal or higher version as us.
        let mut reply = if self.update_versions(msg.metadata_version()) {
            let body = msg.into_body();
            self.merge_local_view(peer, body.payload).await?
        } else {
            // otherwise we assume our view is latest and return everything
            self.nodes.clone()
        };
        ...
        let outgoing = reciprocal.prepare(GossipResponse {
            payload: GossipPayload::Snapshot { nodes: reply },
        });
    ...
    // update known versions of metadata. Return true if our version is equal
    // to or higher than the remote peer version, otherwise returns false.
    fn update_versions(&mut self, peer_versions: &PeerMetadataVersion) -> bool {
        let peer_logs_version = peer_versions.logs.unwrap_or(Version::MIN);
        let peer_partition_table_version = peer_versions.partition_table.unwrap_or(Version::MIN);
        let peer_nodes_config_version = peer_versions.nodes_config.unwrap_or(Version::MIN);

        if peer_logs_version < self.logs_version
            || peer_partition_table_version < self.partition_table_version
            || peer_nodes_config_version < self.nodes_config_version
        {
            return false;
        }

        self.logs_version = self.logs_version.max(peer_logs_version);
        self.partition_table_version = self
            .partition_table_version
            .max(peer_partition_table_version);
        self.nodes_config_version = self.nodes_config_version.max(peer_nodes_config_version);
