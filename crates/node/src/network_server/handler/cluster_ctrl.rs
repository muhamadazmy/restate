...
use restate_admin::cluster_controller::protobuf::{
    ...
    ListLogsResponse, ListNodesRequest, ListNodesResponse, ProviderKind as ProtoProviderKind,
    SealAndExtendChainRequest, TrimLogRequest,
};
...
impl ClusterCtrlSvc for ClusterCtrlSvcHandler {
    ...
    async fn seal_and_extend_chain(
        ...
        let request = request.into_inner();
        let Ok(kind) = ProtoProviderKind::try_from(request.provider) else {
            return Err(Status::invalid_argument("Invalid provider kind"));
        };

        let kind: ProviderKind = kind
            .as_str_name()
            .parse()
            .map_err(|_| Status::unimplemented("Provider type is not supported"))?;
