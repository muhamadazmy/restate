...
impl QueryContext {
    pub async fn create<T: TableRegisterer>(
        ...
        Self::from_iter(options, std::iter::once(boxed)).await
    }

    pub async fn from_iter<I>(options: &QueryEngineOptions, tables: I) -> Result<Self, BuildError>
    where
    ...
    pub async fn with_sys_tables(
        ...
        Self::create(options, sys_tables).await
    }
